...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${isEndIncluded ? '[' : '('}${b}, ${a}${isStartIncluded ? ']' : ')'}`;
}
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(num.toString().split('').reduce((acc, item) => acc + +item, 0));
}
...
function getCommonDirectoryPath(pathes) {
  const len = pathes.slice().sort((a, b) => b.length - a.length)[pathes.length - 1].length;
  const result = [];
...
function evaluateTicTacToePosition(position) {
  ...
  if (position.some((row) => row.every((i) => i === 'X'))) {
    return 'X';
  }

  if (position.some((row) => row.every((i) => i === '0'))) {
    return '0';
  }

  for (let i = 0; i < 3; i += 1) {
    if (position.every((row) => row[i] === 'X')) {
      return 'X';
    }

    if (position.every((row) => row[i] === '0')) {
      return '0';
    }
  }
